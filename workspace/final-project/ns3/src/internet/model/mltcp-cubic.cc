...
#include "mltcp-cubic.h"

#include "ns3/log.h"

NS_LOG_COMPONENT_DEFINE("MLTcpCubic");

namespace ns3
{

NS_OBJECT_ENSURE_REGISTERED(MLTcpCubic);

int COMP_TIME = 1000;
int COMM_DATA = 12500000;

TypeId
MLTcpCubic::GetTypeId()
{
    static TypeId tid =
        TypeId("ns3::MLTcpCubic")
            .SetParent<TcpSocketBase>()
            .AddConstructor<MLTcpCubic>()
            .SetGroupName("Internet")
            .AddAttribute("FastConvergence",
                          "Enable (true) or disable (false) fast convergence",
                          BooleanValue(true),
                          MakeBooleanAccessor(&MLTcpCubic::m_fastConvergence),
                          MakeBooleanChecker())
            .AddAttribute("Beta",
                          "Beta for multiplicative decrease",
                          DoubleValue(0.7),
                          MakeDoubleAccessor(&MLTcpCubic::m_beta),
                          MakeDoubleChecker<double>(0.0))
            .AddAttribute("HyStart",
                          "Enable (true) or disable (false) hybrid slow start algorithm",
                          BooleanValue(true),
                          MakeBooleanAccessor(&MLTcpCubic::m_hystart),
                          MakeBooleanChecker())
            .AddAttribute("HyStartLowWindow",
                          "Lower bound cWnd for hybrid slow start (segments)",
                          UintegerValue(16),
                          MakeUintegerAccessor(&MLTcpCubic::m_hystartLowWindow),
            ...
            .AddAttribute("HyStartDetect",
                          ...
                          MakeEnumAccessor(&MLTcpCubic::m_hystartDetect),
            ...
            .AddAttribute("CntClamp",
                          ...
                          " division",
                          UintegerValue(20),
                          MakeUintegerAccessor(&MLTcpCubic::m_cntClamp),
                          MakeUintegerChecker<uint8_t>())
            .AddAttribute("C",
                          "Cubic Scaling factor",
                          DoubleValue(0.4),
                          MakeDoubleAccessor(&MLTcpCubic::m_c),
                          MakeDoubleChecker<double>(0.0));
    return tid;
}

MLTcpCubic::MLTcpCubic()
    : TcpCongestionOps(),
      m_cWndCnt(0),
      m_lastMaxCwnd(0),
      m_bicOriginPoint(0),
...
MLTcpCubic::MLTcpCubic(const MLTcpCubic& sock)
...
std::string
MLTcpCubic::GetName() const
{
    return "TcpCubic";
}

void
MLTcpCubic::HystartReset(Ptr<const TcpSocketState> tcb)
{
    NS_LOG_FUNCTION(this);
...
void
MLTcpCubic::IncreaseWindow(Ptr<TcpSocketState> tcb, uint32_t segmentsAcked)
{
...
uint32_t
MLTcpCubic::Update(Ptr<TcpSocketState> tcb)
{
    NS_LOG_FUNCTION(this);
    Time t;
    uint32_t delta;
    ...
    float lower = 0.5;
    float upper = 2.0;
    float alpha = lower + (upper - lower) * (m_dataSent / COMM_DATA);
    t = t * alpha;
...
void
MLTcpCubic::PktsAcked(Ptr<TcpSocketState> tcb, uint32_t segmentsAcked, const Time& rtt)
{
    NS_LOG_FUNCTION(this << tcb << segmentsAcked << rtt);
    
    if (Simulator::Now().GetMilliSeconds() > m_lastTimeSent + COMP_TIME) {
        m_dataSent = 0;
    }
    m_dataSent += segmentsAcked * tcb->m_segmentSize;
    m_lastTimeSent = Simulator::Now().GetMilliSeconds();
...
void
MLTcpCubic::HystartUpdate(Ptr<TcpSocketState> tcb, const Time& delay)
{
...
Time
MLTcpCubic::HystartDelayThresh(const Time& t) const
{
...
uint32_t
MLTcpCubic::GetSsThresh(Ptr<const TcpSocketState> tcb, uint32_t bytesInFlight)
{
...
void
MLTcpCubic::CongestionStateSet(Ptr<TcpSocketState> tcb, const TcpSocketState::TcpCongState_t newState)
{
...
void
MLTcpCubic::CubicReset(Ptr<const TcpSocketState> tcb)
{
...
{
    NS_LOG_FUNCTION(this);
    return CopyObject<MLTcpCubic>(this);
}
