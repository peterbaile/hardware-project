...
class TcpSocketBase : public TcpSocket
{
  public:
    ...
    /** 
     * \brief Callback pointer for cWnd trace chaining. Also returns the Ipv4 addresses from the endpoint
     */
    TracedCallback<uint32_t, uint32_t, uint32_t, uint16_t, uint32_t, uint16_t> m_cWndAddrTrace;
    ...
    TracedCallback<Time, Time, uint32_t, uint16_t, uint32_t, uint16_t> m_lastRttAddressTrace;
    ...
    /**
     * Configure parameters for MLTCP sockets.
    */
    void ConfigureMLParameters(Time, uint32_t);
    // temporary:
    void ConfigureLogging(Ipv4Address, Ipv4Address);   
