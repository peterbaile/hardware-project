    ...
    static TypeId tid =
        TypeId("ns3::TcpSocketBase")
            ...
            .AddTraceSource("RTT",
                            ...
                            "ns3::TracedValueCallback::Time")
            .AddTraceSource("RTTAddress",
                            "Last RTT sample and addresses",
                            MakeTraceSourceAccessor(&TcpSocketBase::m_lastRttAddressTrace),
                            "ns3::TracedValueCallback::TimeAddress")
            ...
            .AddTraceSource("CongestionWindow",
                            ...
                            "ns3::TracedValueCallback::Uint32")
            .AddTraceSource("CongestionWindowAddress",
                            "The TCP connection's congestion window and addresses",
                            MakeTraceSourceAccessor(&TcpSocketBase::m_cWndAddrTrace),
                            "ns3::TracedValueCallback::Uint32Addr")
...
TcpSocketBase::UpdateCwnd(uint32_t oldValue, uint32_t newValue) const
{
    ...
    m_cWndAddrTrace(oldValue, newValue, m_endPoint->GetLocalAddress().Get(), m_endPoint->GetLocalPort(), m_endPoint->GetPeerAddress().Get(), m_endPoint->GetPeerPort());
}
...
TcpSocketBase::UpdateRtt(Time oldValue, Time newValue) const
{
    ...
    m_lastRttAddressTrace(oldValue, newValue, m_endPoint->GetLocalAddress().Get(), m_endPoint->GetLocalPort(), m_endPoint->GetPeerAddress().Get(), m_endPoint->GetPeerPort());
}
...
void TcpSocketBase::ConfigureMLParameters(Time compTime, uint32_t commSize) {
    m_tcb->m_compTime = compTime;
    m_tcb->m_commSize = commSize;
}

void TcpSocketBase::ConfigureLogging(Ipv4Address source, Ipv4Address dest) {
    m_tcb->m_source = source;
    m_tcb->m_dest = dest;
}
